...
class TestPlaceOrder:
    ...
    def test_place_order(self, case,init_class):
        ...
        #发请求
        resp = request.send_request(is_upload=case["is_upload"],url=case["url"],method=case["method"],data=data,port=case["port"])
        #接口断言
        response.assert_responsee(response=resp, expected_data=case["expected_data"])
        #提取数据，设置为类属性
        extract.extract_data(response=resp,extract_data=case["extract_data"])
        #数据库断言
